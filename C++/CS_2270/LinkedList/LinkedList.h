...
public:
  ...
  // helper function to Report to call recursively and fill it in
  string ReportRecursive(shared_ptr<node> cursor);
  ...
  // helper function to do recursive portion of Append feature
  void AppendRecursive(shared_ptr<node> new_node, shared_ptr<node> cursor);
  ...
  // helper function to Insert to recursively solve the problem and inset the new node
  void InsertRecursive(int offset, shared_ptr<node> new_node, shared_ptr<node> prev_node, shared_ptr<node> cursor);
  ...
  void RemoveRecursive(int size, shared_ptr<node> cursor, shared_ptr<node> prev_node);
  ...
  // recursive helper function to size
  void SizeRecursive(shared_ptr<node> cursor, int& size);

  // contains returns true if the linked list pointed to by head
  // contains a node with the specified value, false otherwise.
  bool Contains(int data);

  // recursive helper function of Contains
  void ContainsRecursive(shared_ptr<node> cursor, bool& ret, int data);
